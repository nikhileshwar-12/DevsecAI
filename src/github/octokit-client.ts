...
export class GitHubClient {
   ...
   */
  public async fetchPullRequestDiff(owner: string, repo: string, pullNumber: number): Promise<string> {
    try {
  ...
  public async submitPRReview(
    ...
    try {
      ...
      const comments = reviewResult.findings.map((finding: ReviewFinding) => ({
        path: finding.file,
        line: finding.line,
