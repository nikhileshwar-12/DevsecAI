...
export class ArbiterAgent {
     ...
     */
    static synthesize(prId, repoName, securityFindings, performanceFindings, unitTests, agentMetrics) {
        ...
        const filteredFindings = deduplicatedFindings.filter(f => f.confidence >= config.MIN_CONFIDENCE_THRESHOLD);
        // 3. Compute Composite Risk Score (0 - 100)
        const riskScore = this.calculateRiskScore(filteredFindings);
        // 4. Determine Decision
        const decision = this.determineDecision(riskScore, filteredFindings);
        // 5. Generate Markdown PR Summary
        const summary = this.renderMarkdownSummary(repoName, prId, decision, riskScore, filteredFindings, unitTests);
    ...
    static deduplicateFindings(findings) {
        ...
        for (const finding of findings) {
            const isDuplicate = unique.some(existing => existing.file === finding.file &&
    ...
    static determineDecision(riskScore, findings) {
     ...
     */
    static renderMarkdownSummary(repoName, prId, decision, riskScore, findings, unitTests) {
